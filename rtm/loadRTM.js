export async function loadRTM() {
    try {
      ...
      console.log('RTM data:', data);  // Debugging
              ...
              <div class="card">
                ...
                <div id="collapse-${index}" class="collapse show">
                  <div class="card-body">
                    ${testCases.length > 0 ? `
                      <ul>
                        ${testCases.map(testCase => `<li class="test-case">${testCase}</li>`).join('')}
                      </ul>
                ...
                <div id="collapse-spec-${index}" class="collapse show">
                  <div class="card-body">
                    ${requirements.length > 0 ? `
                      <ul>
                        ${requirements.map(requirement => `<li class="requirement">${requirement}</li>`).join('')}
                      </ul>
      ...
      document.getElementById('requirements-search').addEventListener('keyup', function () {
        const value = this.value.toLowerCase();
        const rows = document.querySelectorAll('#rtm-body tr');
        rows.forEach(row => {
          row.style.display = row.innerText.toLowerCase().includes(value) ? '' : 'none';
        });
      });
  
      document.getElementById('spec-map-search').addEventListener('keyup', function () {
        const value = this.value.toLowerCase();
        const rows = document.querySelectorAll('#spec-map-body tr');
        rows.forEach(row => {
          row.style.display = row.innerText.toLowerCase().includes(value) ? '' : 'none';
        });
      });
